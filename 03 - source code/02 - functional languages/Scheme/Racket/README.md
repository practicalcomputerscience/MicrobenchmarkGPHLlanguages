...
However, Chez Scheme programs are by default not meant to be compiled into standalone, binary executables, but to be run like scripts, for example like this with the Chez compiler: _$ scheme --script ./random_streams_for_perf_stats.ss_ (![Chez source code](https://github.com/practicalcomputerscience/MicrobenchmarkGPHLlanguages/blob/main/03%20-%20source%20code/02%20-%20functional%20languages/Scheme/Chez/random_streams_for_perf_stats.ss)), though I finally found a hack: [Making a standalone executable](TBD).
