(*
random_bitstring_and_flexible_password_generator_unleashed.pp
...
build on Ubuntu 24 LTS: $ fpc -Munleashed random_bitstring_and_flexible_password_generator_unleashed.pp

run on Ubuntu 24 LTS:   $ ./random_bitstring_and_flexible_password_generator_unleashed
...
var
  ...
  WITH_SPECIAL_CHARS : Boolean = true;  // 2026-07-20
...
begin
  ...
  try
    ...
    WriteLn($'Bit stream has been written to disk under name:  {file_bits_x}');  // 2026-07-20
  except
    on E: Exception do
      WriteLn($'could not write to file: {file_bits_x} ! -- {E.Message}');  // 2026-07-20
  end;

  // write byte stream to disk:
  try
    bits_hex.SaveToFile(file_bits_hex);
    WriteLn($'Byte stream has been written to disk under name: {file_bits_hex}');  // 2026-07-20
  except
    on E: Exception do
      WriteLn($'could not write to file: {file_bits_hex} ! -- {E.Message}');  // 2026-07-20
  end;
  ...
  pattern := if WITH_SPECIAL_CHARS then TRegExpr.Create('[!-~]') else TRegExpr.Create('[A-Za-z0-9]');  // 2026-07-20
  ...
  WriteLn; WriteLn($'Your password of {N_CHAR} characters is: {pw_chars.DataString}');  // 2026-07-20
...
(* end of random_bitstring_and_flexible_password_generator_unleashed.pp *)
