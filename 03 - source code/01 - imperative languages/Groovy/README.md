...
I decided to (quickly) implement the microbenchmark program also in Groovy as another language, which makes program to be natively executed on the Java Virtual Machine.
...
- use the OpenJDK over a JDK version with Java(TM) SE (Standard Edition) for the Oracle GraalVM, something which was already clear at: [A significantly faster Scala based program](https://github.com/practicalcomputerscience/MicrobenchmarkGPHLlanguages/tree/main/04%20-%20GraalVM#a-significantly-faster-scala-based-program)
...
A zero-cost trick for speedy Groovy programs is to use type definitions and annotations, specifically annotation _@CompileStatic_ on classes and methods.
...
Next, the build configuration file [build.gradle](./build.gradle%20for%20random_streams_for_perf_stats) must be prepared (rename the needed _build.gradle for ..._ file to its real file name _build.gradle_). Again, I manually put its content together like this:
