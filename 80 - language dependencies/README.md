...
C | C for GCC or LLVM, with both requiring a working C++ compiler and having numerous other dependencies | only C++ is self-hosting nowadays, not even C anymore
...
Perl 5 |  -- |
Picat |  -- |
PowerShell | script is compiled into PowerShell bytecode, which is then is then interpreted by the Common Language Runtime, often involving lots of overhead with its object-heavy architecture | --
Prolog, SWI | -- |
Python | -- |
Raku | -- | 
Roc | Rust and Zig |
Rust | self-hosting nowadays; the Rust compiler was written originally in OCaml | [Bootstrapping the compiler](https://rustc-dev-guide.rust-lang.org/building/bootstrapping/intro.html)
