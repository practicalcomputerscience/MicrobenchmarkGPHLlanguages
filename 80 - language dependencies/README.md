...
Clojure | the Clojure compiler (_compile-clj_) is written in Java; JVM (Java Virtual Machine) to run generated JVM bytecode | 
...
Python | -- |
Raku | Rakudo compiler: C for a C compiler and a Perl 5 installation; bootstrapping also includes the help of NQP ("Not Quite Perl") files | [Build requirements (Installing from source)](https://github.com/rakudo/rakudo/blob/main/INSTALL.md#build-requirements-installing-from-source)
...
Scheme, CHICKEN | -- | 
Scheme, Gambit | -- | 
Scheme, Racket | runs on the Chez Scheme runtime system |
...
- C++ (**) (but not C nowadays, where even the Tiny C Compiler looks for another C compiler when configuring it for making (in a Linux); and that C compiler may be very well _gcc_ in a typical Linux system)
- TBD
- TBD
- ...
